...
public:
  ...
  std::array<int, 3> ReverseArray(std::aray<int, 3> InputArray)
  {
    
  }

  std::array<int,3> GetNewBondDirection(std::vector<long long int> BondedAtomsToCurrentAtom, vtkAtomId AtomId)
  {
    /**
    ...
    */
    ...
    {
      return molecule->GetAtom(BondedAtomsToCurrentAtom[0])->GetPosition()
    }
    for (long long int i=0; i<molecule->GetNumberOfAtoms(); i++)
  }
};
