...
virtual void OnLeftButtonDown() override
{
  ...
  cout << this->Interactor->GetEventPosition()[0] << "this is x" << endl;
  cout << this->Interactor->GetEventPosition()[1] << "this is y" << endl;
  cout << this->Interactor->GetEventPosition()[2] << "this is z" << endl;
  ...
  for (long long int i=0; i<NumberOfAtoms; i++)
  {
    ...
    if (VectorMagnitude(DistX, DistY, DistZ) < 1000.0) //need to play a little bit and relaize, how numbers work (different sizes of window etc)  
    {
      ...
      BondedAtomsToCurrentAtom.clear();
  }
    // Forward events
  ...
  std::array<float, 3> GetNewAtomPosition(std::vector<long long int> BondedAtomsToCurrentAtom, vtkIdType AtomId)
  {
    ...
    */
    cout << "AtomId is:" << AtomId << endl;
    cout << "Atomic number is " << molecule->GetAtom(AtomId).GetAtomicNumber() << endl;
